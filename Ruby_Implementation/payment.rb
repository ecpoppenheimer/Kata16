...
*** This documentation hasn't been re-written for the Ruby implementation. ***
...
def helper_check_required_kwargs(cls)
    ...
    for c in cls.ancestors()[..-4] do
        for required_kwarg in c.required_kwargs do
            if all_kwargs.keys().include? required_kwarg then
        ...
        removed_kwargs = removed_kwargs | c.filled_kwargs
    end

    puts "Checking required kwargs for class %s..." % [cls]
    ...
    all_kwargs.each() do |kwarg, classes|
        if classes.length == 1 then
            ...
            unless removed_kwargs.include?(kwarg) then
                puts " | kwarg %s used by class %s" % [kwarg, classes]
            end
        else
            # Whether or not the kwargs is filled anywhere, if it is not unique we need to know.
            puts " X CONFLICT WARNING: kwarg %s used by multiple classes: %s" % [kwarg, classes]
        end
    end
    puts " - Check complete."
    puts "------------------\n"
